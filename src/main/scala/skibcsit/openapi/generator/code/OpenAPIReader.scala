...
object OpenAPIReader {
  def read(location: String): OpenAPI = new OpenAPIParser()
    .readLocation(location, null, null)
    .getOpenAPI

  def getMethods(openAPI: OpenAPI): Iterable[Operation] = openAPI.getPaths
    .entrySet().asScala
    .map(_.getValue)
  ...
  def getFirstSchema(content: Content): Schema[_] = content
    .entrySet.asScala
    .headOption
    .map(_.getValue.getSchema)
    .orNull
}
